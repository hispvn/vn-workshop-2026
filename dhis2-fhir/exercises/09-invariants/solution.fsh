// Exercise 09: Invariants - Solution
// FHIRPath invariants for DHIS2 validation rules.

Invariant: dhis2-uid-format
Description: "DHIS2 UID must be exactly 11 alphanumeric characters starting with a letter"
Severity: #error
Expression: "matches('^[a-zA-Z][a-zA-Z0-9]{10}$')"

Invariant: dhis2-identifier-has-system
Description: "Every identifier must have a system"
Severity: #error
Expression: "system.exists()"

Profile: DHIS2PatientWithInvariants
Parent: Patient
Id: dhis2-patient-invariants
Title: "DHIS2 Patient with Invariants"
Description: "A Patient profile with DHIS2 validation invariants applied."
* identifier 1..* MS
* identifier obeys dhis2-identifier-has-system
* identifier.value obeys dhis2-uid-format
* name 1..* MS
* birthDate 1..1 MS
* gender 1..1 MS
